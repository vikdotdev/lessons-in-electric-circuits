...
function makeEpub(bookName, title) {
  ...
  shell.mkdir('-p', 'epub');
  shell.exec(`pandoc --epub-metadata=epub.txt --epub-cover-image="html/${bookName}/cover.jpg" --metadata title="${title}" ${path.resolve(tempFiles.join(' '))} -o epub/${bookName}.epub`, code => {
    if (code == 0) {
